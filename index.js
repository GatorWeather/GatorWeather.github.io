...
const hurricaneBtn = document.querySelector(".hurricaneBtn");
const hurricaneContainer = document.querySelector(".hurricaneContainer");
...
// ── Hurricane Tracker ──
function showHurricaneBtn() {
    hurricaneBtn.style.display = "flex";
    hurricaneContainer.classList.remove("show");
    hurricaneBtn.classList.remove("open");
...
async function initHurricaneMap() {
    ...
    // inject header
    const header = document.createElement("div");
    header.classList.add("hurricaneHeader");
    header.innerHTML = `
        <div>
            <div class="hurricaneHeaderTitle">Hurricane Tracker</div>
            <div class="hurricaneHeaderSub">NHC active tropical storms</div>
        </div>
        <span class="hurricaneStormCount">${storms.length} active storm${storms.length !== 1 ? "s" : ""}</span>
    `;
    panel.insertBefore(header, panel.firstChild);

    if (storms.length === 0) {
        const noStorms = document.createElement("div");
        noStorms.classList.add("hurricaneNoStorms");
        noStorms.innerHTML = `
            <i class="fa-solid fa-circle-check"></i>
            <strong>No active tropical storms</strong>
            <span>Check back during hurricane season (June–November)</span>
        `;
        panel.appendChild(noStorms);
        document.getElementById("hurricaneMap").style.display = "none";
        return;
    }

    // init map
    const centerLat = storms[0].lat || 20;
    const centerLon = storms[0].lon || -60;

    hurricaneMap = L.map("hurricaneMap", {
        zoomControl: true,
        maxZoom: 10
    ...
    storms.forEach(storm => {
        const category = getStormCategory(storm.maxWind || 0);
...
function getStormCategory(windMph) {
...
function getCategoryLabel(cat) {
    ...
    return labels[cat] || "Tropical System";
}

if (hurricaneBtn) {
    hurricaneBtn.addEventListener("click", async () => {
        const isOpen = hurricaneContainer.classList.contains("show");
        if (isOpen) {
            hurricaneContainer.classList.remove("show");
            hurricaneBtn.classList.remove("open");
        } else {
            hurricaneContainer.classList.add("show");
            hurricaneBtn.classList.add("open");
            if (!hurricaneMap) {
                setTimeout(async () => {
                    await initHurricaneMap();
                }, 100);
            }
        }
    });
...
function parseNHCFeed(xmlString, basin) {
    const storms = [];
    if (!xmlString) return storms;

    try {
        const parser = new DOMParser();
        const xml = parser.parseFromString(xmlString, "text/xml");

        // use nhc:Cyclone tags directly — much more reliable than regex
        const cyclones = xml.querySelectorAll("Cyclone");

        cyclones.forEach(cyclone => {
            const centerText = cyclone.querySelector("center")?.textContent || "";
            const coords = centerText.split(",").map(s => parseFloat(s.trim()));
            if (coords.length < 2 || isNaN(coords[0]) || isNaN(coords[1])) return;

            const lat = coords[0];
            const lon = coords[1];
            const name = cyclone.querySelector("name")?.textContent || "Unknown";
            const type = cyclone.querySelector("type")?.textContent || "";
            const windText = cyclone.querySelector("wind")?.textContent || "0";
            const maxWind = parseInt(windText) || 0;
            const pressureText = cyclone.querySelector("pressure")?.textContent || "";
            const pressure = parseInt(pressureText) || null;
            const headline = cyclone.querySelector("headline")?.textContent || "";
            const movement = cyclone.querySelector("movement")?.textContent || "";

            storms.push({ name, lat, lon, maxWind, pressure, headline, movement, type, basin });
        });

    } catch (e) {
        console.warn("Error parsing NHC feed:", e);
    }
